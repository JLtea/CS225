...
class KDTree
{
  ...
  private:
     ...
     */
    Point<Dim> quickselect(vector<Point<Dim>> &toSort, int medIdx, int from, int to, int currDim);
    ...
    Point<Dim> findNearest(KDTreeNode* subRoot,const Point<Dim>& query, int currDim) const;
